...
function leaderboard(originalMessage, self) {
  self.db.all(
    ...
    (err, denkipoints) => {
      const data = denkipoints.map(denkipointsUser => {
        return [
          self.getUsernameById(denkipointsUser.user_id),
          denkipointsUser.points,
        ];
      });
      if (data.length === 0) {
        self.simpleDenki(
          ...
          `<@${self.getUsernameById(
            originalMessage.user,
          )}> \`!denkipoints <user> to initialize\``,
        );
      } else {
        self.simpleDenki(
          originalMessage,
          `<@${originalMessage.user}> \`\`\`Denkipoints:\n${table(data)}\`\`\``,
        );
