...
function leaderboard(originalMessage, self) {
  self.db.all(
    ...
    (err, sneezepoints) => {
      const data = sneezepoints.map(sneezepointsUser => {
        return [
          self.getUsernameById(sneezepointsUser.user_id),
          sneezepointsUser.points,
        ];
      });
      if (data.length === 0) {
        self.simpleDenki(
          ...
          `<@${self.getUsernameById(
            originalMessage.user,
          )}> \`!sneezepoints <user> to initialize\``,
        );
      } else {
        self.simpleDenki(
          originalMessage,
          `<@${originalMessage.user}> \`\`\`Sneezepoints:\n${table(
            data,
          )}\`\`\``,
        );
